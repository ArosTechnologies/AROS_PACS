...
- [ ] Generar par de llaves RSA (privada/pública) y almacenar en Secrets Manager de `aros-core`.
- [ ] Configurar `djangorestframework-simplejwt` con RS256 y claim `kid` en el header de cada JWT.
- [ ] Implementar endpoint **`GET /api/v1/.well-known/jwks.json`** que publica la llave pública activa.
- [ ] Configurar rotación automática de llave RSA en Secrets Manager (Lambda rotation, período 90 días).
- [ ] Implementar lógica de **soporte para múltiples llaves activas** durante periodos de transición.
- [ ] Implementar **Refresh Token Blacklist** en Redis: guardar JTI en `setex` al logout; verificar en cada uso del refresh endpoint.
- [ ] Configurar `CONN_MAX_AGE = 60` en `settings.py` para connection pooling básico.
- [ ] Implementar cifrado del email con KMS (`email_hash` + `email_encrypted`) en el flujo de registro.
- [ ] Configurar middleware de seguridad y CORS.

### Fase 3 — Federated Query, S2S JWT y Proxy WADO-RS *(Semana 2-3)*

- [ ] Implementar generador de **S2S JWT** (`iss: aros-core`, `aud: clinic-{slug}`, `exp: +5min`) firmado con la llave privada RSA.
- [ ] Integrar `S2S JWT` como header `X-Core-Service-Token` en todas las llamadas httpx del gateway hacia Clinic APIs.
- [ ] Implementar Federated Query con `asyncio.gather()` + `select_related()` en la query de `FederationIDMap` (evitar N+1).
- [ ] Implementar **Timeout** configurable (5s por defecto) en cada llamada httpx a Clinic API.
- [ ] Implementar **Circuit Breaker** con `pybreaker` (un breaker por clínica, `fail_max=5`, `reset_timeout=60`).
- [ ] Implementar lógica de **historial parcial**: retornar `partial_history: true` y `unavailable_sources` cuando alguna clínica falla.
- [ ] Verificar `ConsentRecord` antes de incluir cada clínica en el Federated Query.
- [ ] Implementar proxy WADO-RS con **`StreamingHttpResponse` + `httpx.stream()`** (chunk de 64KB, sin buffering RAM).
- [ ] Configurar proxy WADO-RS hacia los ALBs internos de las clínicas via TGW.

### Fase 4 — Clinic Internal API: S2S JWT, HMAC y Webhooks *(Semana 3-4)*

- [ ] Configurar `clinic-api` para verificar **S2S JWT** en cada request entrante (usando llave pública distribuida por Terraform).
- [ ] Implementar endpoints en `clinic-api` para gestionar `PatientProfile`, `StudyRequest` y `Report`.
- [ ] Implementar receptor de Webhooks en `clinic-api` con validación **HMAC-SHA256** del payload usando `hmac.compare_digest`.
- [ ] Retornar `HTTP 401` si el header `X-Orthanc-Webhook-Signature` falta o es inválido.
- [ ] Dockerizar el `clinic-api` con health checks configurados en Dockerfile.

### Fase 5 — Configuración de Orthanc PACS con StorageCache *(Semana 4)*

- [ ] Configurar `orthanc.json` con bucket `orthanc-{slug}-{aws_account_id}-dicom`.
- [ ] Activar **`StorageCache`** con 512MB en la configuración de Orthanc para minimizar lecturas de S3.
- [ ] Configurar webhook `OnStableStudy` con el header `X-Orthanc-Webhook-Secret` y el campo `Secret` para HMAC-SHA256 del payload.
- [ ] Verificar que el Clinic API rechaza con `HTTP 401` webhooks con firma HMAC inválida.
- [ ] Verificar health check: `GET /` en puerto 8042 responde en < 200ms con plugins cargados.
- [ ] Confirmar que el bucket S3 está en la **misma región AWS** que el ECS task de Orthanc (latencia mínima S3).

### Fase 6 — Portales Frontend *(Semana 5-7)*

- [ ] Conectar Portal Clínica y Portal Paciente al AROS Core API.
- [ ] Implementar banner de **"historial parcial"** en el Portal Paciente cuando `partial_history: true`.
- [ ] Configurar OHIF con prefetching de 10 frames (`maxNumRequests.prefetch = 10`).
- [ ] Renderizar visor OHIF utilizando la ruta proxy WADO-RS del Core API (con streaming).
- [ ] Implementar flujo de UI para gestión de `ConsentRecord` (autorizar/revocar clínicas).
