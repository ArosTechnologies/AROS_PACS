...
The application interfaces with an external PACS/RIS system for image synchronization and study retrieval.

## 2. Project Layout
The main source directory is `arosPacs/`, which contains the following key components:

- **`arosPacs/` (Configuration App):** 
  - `settings.py`: Contains all system settings, database configurations, allowed hosts, static/media file routing (S3 vs local).
...
- **Docker Compose:** Used to spin up the local/production environment consisting of:
  ...
  - `sync_pacs`: A background worker running a custom management command (`sync_pacs_images`) to pull incoming images from the PACS.
